...
namespace EnigmaV2
{
    class rdamodule
    {
        ...
        public byte[] sDesymmetricate(byte[] word)
        {
            int remainder = (word[0] - 0x20);
            byte[] asymmetric = new byte[word.Length - 1 - remainder];
            for (int i = 1; i < asymmetric.Length + 1; i++)
            {
